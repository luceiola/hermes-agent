...
目标：在同一个仓库下运行多个隔离 profile（`personal` / `heidou` / `mei`），分别接入不同消息平台。

## 设计原则
- 复用你现有 `hermes-agent` 的网关运行方式（与 `hermes-runtime` 同实现风格）
- profile 隔离：会话、日志、技能、环境变量相互独立
- 默认 profile：`personal`（你）、`heidou`（孩子）、`mei`（媳妇）
- 默认 dashboard 端口：`9129` / `9130` / `9131`
- 平台接入遵循官方渠道：QQ Bot（官方 API v2）、Weixin（官方 iLink Bot）

## 目录
- `profiles/personal|heidou|mei/`: 三套 profile 模板
- `skills/personal|heidou|mei/`: 三套技能入口
- `scripts/`: 启停、初始化、同步脚本
- `docs/`: 架构与运维文档
...
只要保持下列项不同，就不会冲突：
- `HERMES_PROFILE`（建议固定 `personal|heidou|mei`）
- 平台凭证（不要跨 profile 复用同一 bot 凭证）
- `HERMES_DASHBOARD_PORT`（`9129/9130/9131`）
